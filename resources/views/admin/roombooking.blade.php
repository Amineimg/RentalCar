        ...
        <div class="table-responsive">
            <table class="table bordered striped">
                ...
                <tr>
                    ...
                    <th>{{get_string('room')}}</th>
                    <th>{{get_string('start_date')}}</th>
                    <th>{{get_string('end_date')}}</th>
                    <th>{{get_string('passengers_number')}}</th>
                    ...
                    <tr class="{{ $booking->completed ? 'disabled-style' : '' }}">
                        ...
                        <td class="text-right">
                            <div class="icon-options">
                                @if($booking->fully_booking && $booking->fully_booking == 1)
                                ...
                                @else
                                    @if($booking->completed && $booking->completed == 1)
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.user-info').click(function(e){
            ...
            var token = $('.token').val();
            $.ajax({
                url: '{{ url('/admin/roombooking/user_details') }}/' + id,
                type: 'post',
                data: {_token: token},
            ...
            //if(!selector.hasClass('disabled-style')) {
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            $.ajax({
                                url: '{{ url('/admin/roombooking/activate') }}/' + id,
                                type: 'post',
                                data: {_token: token},
                                beforeSend: function(){
            ...
            //if(!selector.hasClass('disabled-style')) {
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        ...
                        if (result) {
                            $.ajax({
                                url: '{{ url('/admin/roombooking/reject') }}/' + id,
                                type: 'post',
        ...
        $('.delete-button').click(function(event){
            ...
            bootbox.confirm({
                ...
                callback: function (result) {
                    if(result){
                        $.ajax({
                            url: '{{ url('/admin/roombooking/delete') }}/'+id,
                            type: 'post',
                            data: {_token :token},
                            success:function(msg) {
                                selector.remove();
