...
@section('title')
    <title>{{get_string('edit_carmake') . ' - ' . get_setting('site_name', 'site')}}</title>
@endsection

@section('content')
@section('page_title')
    <h3 class="page-title mbot10">{{get_string('edit_carmake')}}</h3>
@endsection
<div class="col s12">
    @if(!$errors->isEmpty())
        <span class="wrong-error">* {{get_string('validation_error')}}</span>
    @endif
        {!! Form::open(['method' => 'patch', 'url' => route('admin.taxonomy.carmake.update', $carmake->id), 'files' => 'true']) !!}
    <div class="panel">
                                    ...
                                    <div id="lang{{$language->id}}" class="tab-pane {{$language->default ? 'active' : ''}}">
                                        <div class="col s12">
                                            <div class="form-group  {{$errors->has('name.'.$language->id.'') ? 'has-error' : ''}}">
                                                {{Form::text('name['.$language->id.']', $carmake->content($language->id)->name, ['class' => 'form-control', 'placeholder' => get_string('enter_carmake_name')])}}
                ...
                <div id="data-panel" class="tab-pane">
                    <div class="col s12">
                         <div class="form-group  {{$errors->has('alias') ? 'has-error' : ''}}">
                            {{Form::text('alias', $carmake->alias, ['class' => 'form-control', 'placeholder' => get_string('alias')])}}
                    ...
                    <div class="col s12">
                        <div class="form-group">
                            {{Form::number('order', $carmake->order, ['class' => 'form-control', 'min' => '0', 'step' => 1, 'placeholder' => get_string('order')])}}
                    ...
                    <div class="col s12">
                        ...
                        <div class="form-group  {{$errors->has('map_icon') ? 'has-error' : ''}}">
                            {{Form::text('map_icon', $carmake->map_icon, ['class' => 'form-control', 'placeholder' => get_string('map_icon')])}}
                    ...
                    @if($carmake->featured_image)
                    <div class="col l4 m4 s4">
                        <a href="#!" class="delete-featured-image btn waves-effect btn-red" data-id="{{$carmake->id}}"><i class="material-icons color-white">delete</i>{{ get_string('delete_image') }}</a>
                    </div>
                        <img id="featured-image" class="img-responsive mtop10" src="{{$carmake->featured_image}}"/>
                    @endif
                </div>
            </div>
    ...
    <script>
        $(document).ready(function(){
            $('.desc-content').summernote({
                ...
                maxwidth: false,
                minwidth: false,
                placeholder: '{{get_string('enter_carmake_content')}}',
                        ...
                        if(result){
                            $.ajax({
                                ...
                                success:function(msg) {
                                    window.location = "/admin/taxonomy/carmake";
                                }
                            });
                        }
                    }
            ...
            $('.delete-featured-image').click(function(event){
                ...
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if(result){
                            $.ajax({
                                url: '{{ url('/admin/taxonomy/carmake/deleteImage') }}/'+id,
                                type: 'post',
                                data: {_token :token},
                                success:function(msg) {
