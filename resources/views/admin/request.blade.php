...
<div class="col s12">
    ...
    @endif
    {{ csrf_field() }}
    <h3 class="page-title">{{get_string('cars')}}</h3>
    @if($cars->count())
        <div class="table-responsive">
            <table class="table bordered striped">
                <thead class="thead-inverse">
                ...
                @foreach($cars as $car)
                    <tr>
                        ...
                        <td>
                            <div class="icon-options">
                                <a href="#" data-type="3" class="activate-button {{$car->status ? 'hidden': ''}}" data-id="{{$car->id}}" title="{{get_string('activate_car')}}"><i class="small material-icons color-primary">done</i></a>
                            </div>
                        </td>
                    </tr>
                @endforeach
        ...
        $(document).ready(function(){
            $('.activate-button').click(function(event){
                ...
                switch(type){
                    ...
                    case 3 : var url = '{{ url('/admin/car/activate/') }}/'; break;
