...
@section('content')
@section('page_title')
    <h3 class="page-title mbot10">{{get_string('activities')}} - {{ $user }}</h3>
@endsection
            ...
            <table class="table bordered striped">
                ...
                @foreach($activities as $activity)
                    <tr class="{{ $activity->completed ? 'disabled-style' : '' }}">
                        ...
                        <td>@if($activity->car_id && $activity->car) {{ $activity->car->contentDefault->name }} @elseif($activity->service) {{ $activity->service->contentDefault->name }} @endif</td>
