...
@section('title')
    ...
    <meta car="og:title" content="{{$static_data['strings']['location'] . ' - ' . $location->contentload->location}}" />
    <meta car="og:image" content="{{asset('/assets/images/home/').'/'.$static_data['design_settings']['slider_background']}}" />
    <meta car="og:description" content="{{strip_tags($location->contentload->description)}}">
@endsection
@section('head')
            ...
            <div class="tab-content">
                <div class="tab-pane active" id="accordion-cars" role="tabpanel">
                    <div class="row">
                        @if(count($cars))
                            <div class="col-sm-12">
                                <h3 class="section-type text-uppercase">{{ $static_data['strings']['cars'] }}</h3>
                            </div>
                            <div class="col-sm-12 filter-box">
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        ...
                                        <input type="text" readonly name="passengers_number_value" value="" class="form-control filter-field" placeholder="{{$static_data['strings']['passengers_number']}}">
                                    </div>
                                    <input type="hidden" name="passengers_number" value="0" class="form-control filter-hidden hidden" placeholder="{{$static_data['strings']['choose_your_category']}}">
                                    <ul class="dropdown-filter-menu">
                                        @for ($i = 1; $i <= get_setting('passengers_number_max','car'); $i++)
                                            <li data-number="{{$i}}">
                                                <a href="#" class="passengers_number_picker">
                                                    <span>{{ $i }}</span>
                                                </a>
                                            </li>
                                        @endfor
                                ...
                                <div class="form-group">
                                    ...
                                    <ul class="dropdown-filter-menu">
                                        @for ($i = 1; $i <= get_setting('rooms_number_max','car'); $i++)
                                            <li data-number="{{$i}}">
                                ...
                                <div class="form-group not-after price-range">
                                    <p>{{$static_data['strings']['price_range']}}: <span class="min">0</span> -  <span class="max">{{get_setting('price_range_max', 'car')}}</span>{{ userCurrencySymbol() }}</p>
                                    <input type="hidden" value="0" class="hidden" name="price_min"/>
                                    <input type="hidden" value="0" class="hidden" name="price_max"/>
                                    <div id="price-range"></div>
                                </div>
                                <a href="#" class="primary-button car-filter">{{$static_data['strings']['search']}}</a>
    ...
    <script type="text/javascript">
        $('document').ready(function(){
            ...
            $('.load-more-cars').click(function(e){
                ...
                e.preventDefault();
                $.ajax({
                    url: '{{ url('location') .'/'. $location->id .'/cars?page=' }}' + page_car,
                    beforeSend: function(){
