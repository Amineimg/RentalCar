...
@section('title')
    ...
    <meta car="og:title" content="@if($service->meta_title) {{ $service->meta_title }} @else {{$static_data['strings']['service'] .' - '. $service->contentload->name}} @endif" />
    <meta car="og:image" content="@if(count($service->images)) {{ asset('images/data').'/'.$service->images[0]->image }} @else{{asset('/assets/images/home/').'/'.$static_data['design_settings']['slider_background']}} @endif" />
    <meta car="og:description" content="@if($service->meta_description) {{ $service->meta_description }} @else {{strip_tags(str_limit($service->contentload->description, 200))}} @endif">
@endsection
@section('bg')
    @if(count($service->images) && get_setting('show_first_image', 'car')) 
        {{ asset('images/data').'/'.$service->images[0]->image }} 
    @else
            ...
            <div class="features">
                ...
                @foreach($features as $feature)
                    <div class="col-md-2 col-sm-3 amenity">
                        ...
                        @elseif(!get_setting('show_available_amenities_only', 'car'))
