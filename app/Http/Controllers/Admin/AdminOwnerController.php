...

{
    ...
    public function delete(Request $request, $id){
        ...
        if($request->ajax()) {
            ...
            if(Car::where('user_id', $user->user_id)->first() || Service::where('user_id', $user->user_id)->first() || Review::where('user_id', $user->user_id)->first() || Booking::where('user_id', $user->user_id)->first()){
    ...
    public function allcars($id){
        ...
        $cars = Car::where('user_id', $id)->orderBy('created_at','desc')->paginate(10);

        $user = Owner::where('user_id', $id)->first()->user->username;

        return view('admin.car.user_index', compact('cars', 'user'));
