...
class PaymentController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($days){
            $price = 0;
            switch($days){
                case $days >= 30: $price = isset($car->prices['d_14']) ? $car->prices['d_14'] : $car->price_per_night; break;
                case $days >= 7: $price = isset($car->prices['d_7']) ? $car->prices['d_7'] : $car->price_per_night; break;
                default: $price = $car->price_per_night; break;
            }
            // Calculate Total and Fees
            $total = $price * $days; 
            $fees = 0 ;
            if(isset($car->fees['city_fee'])){
                $fees += $car->fees['city_fee'];
            }
            if(isset($car->fees['cleaning_fee'])){
                $fees += $car->fees['cleaning_fee'];
            }
            ...
            // Return View back
            return view('home.payment.index', compact('gateways', 'email', 'first_name', 'phone', 'owner_id', 'car_id', 'user_id', 'car', 'currency', 'static_data', 'passengers_number', 'default_language', 'start_date', 'end_date', 'total', 'passengers_number', 'fees', 'grand_total'));
        }else{
    ...
    public function payment(Request $request)
    {
        ...
        $data['buy_points'] = get_string('pay_for_your_book') .' - '. $request->car_name;
        $data['description'] = get_string('booking') . ' - '. $request->car_name;
        ...
        if($request->gateway == 0){
            ...
            if ($response->isSuccessful()) {
                ...
                if($stripeResponse['paid'] && $stripeResponse['status'] === 'succeeded'){
                    ...
                    $mail_data['str_passengers_number'] = $static_data['strings']['passengers_number'];
