...
if(!function_exists('setActive')){
    ...
    {
        $request = Request::path();//var_dump($request);die();
...
if(!function_exists('userCurrencySymbol')){
    function userCurrencySymbol(){
        ...
        $currency = Currency::where('code',$code)->first()->toArray();
...
if(!function_exists('get_min_price')){
    function get_min_price($car_id){
        $seasons = Season::where('car_id', $car_id)->get();
        $min_price = 0;
        foreach ($seasons as $season) {
            if((empty($min_price) && !empty($season->prices['d_14'])) || (!empty($min_price) && !empty($season->prices['d_14']) && $min_price > $season->prices['d_14'])){
                $min_price = $season->prices['d_14'];
            }elseif((empty($min_price) && !empty($season->prices['d_7'])) || (!empty($min_price) && !empty($season->prices['d_7']) && $min_price > $season->prices['d_7'])){
                $min_price = $season->prices['d_7'];
            }elseif((empty($min_price) && !empty($season->price_per_night)) || (!empty($min_price) && !empty($season->price_per_night) && $min_price > $season->price_per_night)){
                $min_price = $season->price_per_night;
            }
        }
...
if (!function_exists('reCaptcha')){
    function reCaptcha($recaptcha){
        $secret = '6Lfr53kiAAAAAOm3G0joSiBP0mBFkHIdoDtoaa8K';
